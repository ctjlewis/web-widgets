...
export class PageScrollIndicator extends StatefulWidget {
  static get styles() {
    return `
  ...
  initState() {
    ...
    document.addEventListener('scroll', () => {
      ...
      if (!mutexLock) {
        ...
        /** Do before next frame. */
        requestAnimationFrame(() => {
          /** Set to current scroll %. */
          this.element.style.setProperty(
              'width',
              `${100 * scrollPos / (scrollHeight - clientHeight)}%`,
          );

          /** Unlock after paint. */
          setTimeout(() => mutexLock = false, 0);
        });
      }
      /** Use passive listener for onScroll. */
    }, { passive: true });
  }
}
