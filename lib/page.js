...
/** A bar that increases in width as the page is scrolled. */
export class ProgressWidget extends StatefulWidget {
  static get tag() {
    return 'progress';
  }
  ...
  build() {
    ...
    requestAnimationFrame(() => {
      const sourceClass = this.constructor.name;
      const fromElement = 'document.currentScript.parentNode';
      const expression = `${sourceClass}.from(${fromElement}).initState();`;
  ...
  initState() {
    document.addEventListener('scroll', () => {
      ...
      window.addEventListener('scroll', (e) => {
        ...
        lastKnownScrollPos = scrollEl.scrollTop / scrollEl.clientHeight;

        if (!ticking) {
          window.requestAnimationFrame(() => {
            this.element.setAttribute('value', lastKnownScrollPos);
            ticking = false;
          });
...
export default new HTMLWidget(
    ...
    new BodyWidget(
        new ProgressWidget(),
        new PageView(
            new Inflate().setStyles('background: #555'),
            new Inflate(),
        ),
    ),
);
