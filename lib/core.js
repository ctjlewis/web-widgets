...
export class Widget extends Inheritable {
  ...
  static from(el) {
    ...
    /**
     * @todo
     * Investigate why this code is not required.
     */

    // const thisClassName = this.name;
    // const encodedClassName = el.getAttribute('constructor');

    /**
     * If there is a `constructor` attr, grab constructor from global
     * namespace. Otherwise, use this class as the constructor.
     */
    // if (encodedClassName && thisClassName !== encodedClassName) {
    //   debugLog(
    //       `Called ${thisClassName}.from but element has encoded constructor `
    //       + `name ${encodedClassName} in constructor attribute. Calling ` +
    //       `${encodedClassName}.from(el) instead.`,
    //   );
    //   const ConstructorRef = window[encodedClassName];
    //   const fromRef = deanonymize(
    //       ConstructorRef,
    //       ConstructorRef.from,
    //   );
    //   return ConstructorRef[fromRef](el);
    // }
  ...
  /**
   * Render into an element (optional) and return that element.
   *
  ...
  renderElement(target) {
     ...
     *
     * @type {HTMLElement|HTMLUnknownElement}
     */
    ...
    if (this.attributes)
      for (const [ key, val ] of Object.entries(this.attributes)) {
        ...
        if (window.FREEZE_MODE && key === 'constructor') continue;
        /** Otherwise set the attribute. */
        renderedElement.setAttribute(key, val);
      }

    /** Apply classes. */
    if (this.classNames.length)
      renderedElement.className = this.classNames.join(' ');

    /** Apply styles. */
    if (this.styles)
      renderedElement.setAttribute(
          'style',
          this.styles.replace(/(\s{2,}|\n+)/g, ' ').trim(),
      );
    ...
    if (this.children.length)
      this.children.forEach(
          (child) => {
            ...
            switch (typeof child) {
              case 'string':
                child = new TextNodeTest(child);
                break;
            }
            renderedElement.appendChild(child.renderElement());
          },
      );


    /**
     * Return rendered element. Replace `target` if provided.
     */
    if (target) target.replaceWith(renderedElement);
    this.constructor.exportStylesTest();
