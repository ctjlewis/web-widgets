...
export class Widget extends Inheritable {
  ...
  constructor(...children) {
     ...
     * `widget-class` attribute on the element.
     */
    ...
    /**
     * Set the `widget-class` attribute.
     */
    this.attributes['widget-class'] = this.constructor.name;
  ...
  static from(el) {
    ...
    const encodedClassName = el.getAttribute('widget-class');

    /**
     * If there is a `widget-class` attr, grab constructor from global
    ...
    if (encodedClassName && thisClassName !== encodedClassName) {
      debugLog(
          ...
          `name ${encodedClassName} in widget-class attribute. Calling ` +
  ...
  applyAttributes() {
    ...
    if (this.attributes) {
      for (const [ key, val ] of Object.entries(this.attributes)) {
        /** If we're freezing a Widget, don't render the widget-class attr. */
        if (window['FREEZE_MODE'] && key === 'widget-class') continue;
