 ...
 *
 * @typedef {Widget|string|number} WidgetLike
 */
...
export class Widget extends Inheritable {
   ...
   */
  freeze() {
    return this.render().element.outerHTML;
  }
...
export class StatefulWidget extends Widget {
  ...
  /**
   * Build the widget and embed the setState call.
   *
   * @param {Node?} target
   * The DOM Node to render into.
   *
   * @param {boolean?} embed
   * Whether or not to embed the state logic.
   *
   * @return {Widget|StatefulWidget}
   */
  render(target = this.element, embed = true) {
    super.render(target);
  ...
  /**
   * Override the `build` method to run `initState` before building.
   *
   ...
   */
  initState() {
    debugLog('StatefulWidget initState', this);
    // debugger;
    // this.build();
    // super.render();
  }
