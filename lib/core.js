...
/**
 * Widgets contain metadata about how to construct a UI element. They contain a
 * `Widget.build` method which returns a `Widget` (by default, `this`) which is
 * then rendered to a DOM Node with `Widget.render`.
 *
 * @category Core
 */
export class Widget extends Inheritable {
  ...
  constructor(...children) {
     ...
     * Move to render().
     */
    if (!(this.constructor.name in globalThis))
  ...
  static exportStyles() {
    ...
    if (EXPORTED_CLASSES.includes(className)) {
    ...
    /** Otherwise, if there are styles to add, add them. */
      ...
      EXPORTED_STYLES.push(declaration);
      EXPORTED_CLASSES.push(className);
    }
  ...
  applyAttributes() {
    ...
    if (DEV) debugLog('applyAttributes', arguments);
  ...
  applyClasses() {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('applyClasses', arguments);
  ...
  applyStyles() {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('applyStyles', arguments);
  ...
  build() {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('build', this.constructor.name);
  ...
  createElement() {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('createElement', arguments);
  ...
  constructElement() {
    /**
     * Export all inheritable class styles to the global namespace, unless we're
     * compiling a namespace in release mode (in which case, the styles will be
     * hydrated from the DOM node's `styles` attr).
     */
  ...
  render(target = this.element) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('Rendering', arguments);
  ...
  replaceElement(node) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('replaceElement', arguments);
  ...
  replaceText(text) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('setTextNode', arguments);
  ...
  setAttributes(attributes = {}) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('setAttributes', arguments);
  ...
  setClasses(...args) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('setClass', arguments);
  ...
  setStyles(...styleStrings) {
    /* eslint-disable-next-line prefer-rest-params */
    if (DEV) debugLog('setStyles', arguments);
...
export class StatefulWidget extends Widget {
  ...
  embedStateLogicIfFrozen() {
    ...
    if (this.frozen) {
      ...
      const expression = (
        ...
        `${dereference(globalThis, sourceClass)}` +
          `.${
  ...
  setState(fn) {
    return () => {
      /** Call provided function with `this.state` ref as argument. */
      fn(this.state);
