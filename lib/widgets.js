...
export class BodyWidget extends FadeIn {
  ...
  build() {
    ...
    this.append(
        /**
         * We need to render a frame in the blank state in order for CSS
         * transitions to work as expected; for the test page, this is just a
         * fade-in animation.
         *
         * The hydration process (`Widget.from`) sometimes renders so quickly
         * that the frame `requestAnimationFrame` gives us is the *first* frame,
         * i.e. the first paint will contain the page in its *final* landing
         * state, leading to the initial CSS transition not firing.
         *
         * Double-wrapping with rAF is ugly but necessary in this unique case,
         * to ensure there is a first paint to transition *from* for any
         * first-frame transitions.
         */
        new Script(
            `requestAnimationFrame(` +
            `function(){requestAnimationFrame(function(){for(var b=${
              deanonymize(window, window.HYDRATE_STACK)
            }||[];b.length;)b.pop()();});});`,
        ),
    );
    return this;
 ...
 */
export class DebugScript extends Script {
  /**
